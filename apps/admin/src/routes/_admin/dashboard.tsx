...
function DashboardPage() {
	...
	const orders: Order[] =
		apiOrders && apiOrders.length > 0 ? (apiOrders as Order[]) : [];
	...
	const filteredOrders = useMemo(() => {
		return orders.filter((o) => {
			...
			if (activeTab === "selesai")
				return (
					matchesSearch && matchesStatus && o.current_status === "ORDER_SELESAI"
				);
			if (activeTab === "aktif")
				return (
					matchesSearch && matchesStatus && o.current_status !== "ORDER_SELESAI"
			...
			if (activeTab === "pending")
				return matchesSearch && matchesStatus && o.current_status === "PENDING";
	...
	const stats = useMemo(
		() => ({
			total: orders.length,
			produksi: orders.filter((o) => o.current_status !== "ORDER_SELESAI")
				.length,
			pending: orders.filter((o) => o.current_status === "PENDING").length,
			selesai: orders.filter((o) => o.current_status === "ORDER_SELESAI")
				.length,
		}),
		[orders],
	);
	...
	return (
		<div className="space-y-8">
				...
				<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
						...
						<div className="w-full lg:w-fit">
							...
							>
								<TabsList className="w-full lg:w-fit flex">
									<TabsTrigger value="semua" className="flex-1 lg:flex-none">
										Semua
									</TabsTrigger>
									<TabsTrigger value="aktif" className="flex-1 lg:flex-none">
										Aktif
									</TabsTrigger>
									<TabsTrigger value="pending" className="flex-1 lg:flex-none">
										Pending
									</TabsTrigger>
									<TabsTrigger value="selesai" className="flex-1 lg:flex-none">
										Selesai
									</TabsTrigger>
								</TabsList>
							</Tabs>
					...
					{orders.length > 0 && (
						<Button
							...
							className="w-full lg:w-auto gap-2 bg-primary hover:bg-primary/90 text-primary-foreground h-10 px-6 font-semibold shadow-sm"
						>
			...
			>
				<SheetContent
					side="right"
					className="w-full sm:max-w-xl overflow-y-auto p-8 sm:p-10"
				>
					...
					<div className="grid gap-6 pt-8">
						...
						<Button
							...
							onClick={() => {
								if (editOrder) {
									toast.success(
										"Order diperbarui (simulasi — API edit belum tersedia)",
									);
									setEditOrder(null);
								} else {
									createOrder();
			...
			>
				<SheetContent
					side="right"
					className="w-full sm:max-w-2xl overflow-y-auto p-8 sm:p-10"
				>
						...
						<div className="grid gap-6 pt-6">
							<div className="space-y-4 rounded-lg bg-muted/50 p-4 border border-border">
								<div className="grid grid-cols-2 gap-4">
									...
									<div>
										...
										<div>
											<Badge
												className={`${STATUS_LABELS[selectedOrderDetails.current_status].color} border-none`}
											>
												{
							...
							<div className="space-y-4">
								...
								<div className="grid gap-4">
									<div className="grid gap-2">
										<Label className="text-xs text-muted-foreground uppercase font-medium tracking-widest">
											Ubah Status
										</Label>
										<select
									...
									{(newStatus === "PENDING" ||
										(selectedOrderDetails.current_status === "PENDING" &&
											!newStatus)) && (
										<div className="p-4 rounded-xl bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-900/40 space-y-3 animate-in fade-in slide-in-from-top-2">
											<div className="grid gap-2">
												<Label className="text-xs text-orange-900/70 dark:text-orange-200/70">
													Deskripsi Kendala
												</Label>
												<Textarea
