...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    try {
      const res = await apiClient.api.login.$post({ json: { email, password } });
      if (!res.ok) return false;
      const data = await res.json() as { data: { id: number; username?: string; name?: string; email: string }; token: string };
      const u: User = {
        id: data.data.id,
